    ...
    do i = 1, kmaxe
      ...
      if (ielem(n, i)%interior .eq. 0) then
        ...
        do l = 1, ielem(n, i)%ifca
          ...
          if (dg .eq. 1)
            weights_dg(1:iqp) = weights_temp(1:iqp)
          do ngp = 1, iqp
            pointx = ngp
            if (dg .eq. 1) then
      ...
      do l = 1, ielem(n, i)%ifca !for all their faces
        ...
        do ngp = 1, iqp        !for all the gaussian quadrature points
          ...
          if ((turbulence .eq. 1) .or. (passivescalar .gt. 0)) then
            ...
            godflux2(nof_variables+1:nof_variables+turbulenceequations+passivescalar)=godflux2(nof_variables+1:nof_variables+turbulenceequations+passivescalar)+(rhllcflux(nof_variables + 1:nof_variables + turbulenceequations + passivescalar)*(weights_temp(ngp)*ielem(n, i)%surf(l)))
          end if
        end do
      ...
      do l = 1, ielem(n, i)%ifca !for all their faces
        ...
        do ngp = 1, iqp        !for all the gaussian quadrature points
          ...
          if ((turbulence .eq. 1) .or. (passivescalar .gt. 0)) then
            ...
            godflux2(nof_variables+1:nof_variables+turbulenceequations+passivescalar)=godflux2(nof_variables+1:nof_variables+turbulenceequations+passivescalar)+(hllcflux(nof_variables + 1:nof_variables + turbulenceequations + passivescalar)*(weights_temp(ngp)*ielem(n, i)%surf(l)))
          end if
        end do
        rhs(i)%val(1:nof_variables) = rhs(i)%val(1:nof_variables) - godflux2(1:nof_variables)
        if ((turbulence .eq. 1) .or. (passivescalar .gt. 0)) then
        ...
        call calculate_bounded_viscous2d(n,b_code,iconsidered,facex,pointx,leftv,rightv,pox,poy,poz,angle1,angle2,nx,ny,nz,cturbl,cturbr,cright_rot,cleft_rot,srf_speedrot,cleft,cright,lcvgrad,rcvgrad,lcvgrad_t,rcvgrad_t)
          ...
          if ((turbulence .eq. 1) .or. (passivescalar .gt. 0)) then
            if (turbulence .eq. 1) then
              ...
              hllcflux(nof_variables + 1:nof_variables + turbulenceequations + passivescalar) = &
              ((oo2*(viscl(1) + viscl(2))))* &
              (((lcvgrad_t(1:turbulenceequations + passivescalar, 1) + rcvgrad_t(1:turbulenceequations + passivescalar, 1))*oo2*nx) + ((lcvgrad_t(1:turbulenceequations + passivescalar, 2) + rcvgrad_t(1:turbulenceequations + passivescalar, 2))*oo2*ny))
            end if
            if (turbulencemodel .eq. 1) then
              hllcflux(5) = hllcflux(5)/sigma
            end if
