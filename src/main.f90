...
!$OMP MASTER
IF (LOWMEM.EQ.0)
    CALL GLOBALISTX(N,XMPIE,XMPIL,XMPIELRANK,IMAXE,ISIZE,CENTERR,GLNEIGH,IELEM)
IF (LOWMEM.EQ.1)
    CALL GLOBALIST(N,XMPIE,XMPIL,XMPIELRANK,IMAXE,ISIZE,CENTERR,GLNEIGH,GLNEIGHPER,IELEM)
!$OMP END MASTER
!$OMP BARRIER
CALL MPI_BARRIER(MPI_COMM_WORLD,IERROR)
...
IF (ISCHEME.GT.1)THEN
    ...
    !$OMP PARALLEL DEFAULT(SHARED)
    IF (LOWMEM.EQ.0)
        CALL DETSTENX(N,ISIZE,IELEM,ISELEMT,XMPIELRANK,ILOCALALLS,TYPESTEN,ILOCALALLELG,STCON,STCONC,STCONS,STCONG,ISOSA,IX,&
        IISTART,IFSAT,PARE,DOSE,PAREEL,DOSEEL,PARES,SOSEEL,XMPIE,IFIN,TFIN,XMPIL,GLNEIGH)
    IF (LOWMEM.EQ.1)
        CALL DETSTEN(N,ISIZE,IELEM,ISELEMT,XMPIELRANK,ILOCALALLS,TYPESTEN,ILOCALALLELG,ILOCALALLELGPER,STCON,STCONC,STCONS,STCONG,ISOSA,IX,&
    ...
    IF ((EES.EQ.0).OR.(EES.GE.4))THEN
        IF (LOWMEM.EQ.0)
            CALL STENCIILSX(N,IELEM,ILOCALALLELG,TYPESTEN,ILOCALSTENCIL,NUMNEIGHBOURS,ISELEMT,XMPIE,XMPIELRANK,ISIZE,BC,VC,VG,ISATISFIED,&
            IWHICHSTEN,IPERIODICITY,XPER,YPER,ZPER,ISSF,ISHYAPE,XMPIL,CENTERR)
        IF (LOWMEM.EQ.1)
            CALL STENCIILS(N,IELEM,ILOCALALLELG,ILOCALALLELGPER,TYPESTEN,ILOCALSTENCIL,ILOCALSTENCILPER,NUMNEIGHBOURS,ISELEMT,XMPIE,XMPIELRANK,ISIZE,BC,VC,VG,ISATISFIED,&
            IWHICHSTEN,IPERIODICITY,XPER,YPER,ZPER,ISSF,ISHYAPE,XMPIL,CENTERR)
    end if
    if ((ees.gt.0).and.(ees.le.2))then
        IF (LOWMEM.EQ.0)
            CALL STENCIILS_EESX(N,IELEM,ILOCALALLELG,TYPESTEN,ILOCALSTENCIL,NUMNEIGHBOURS,ISELEMT,XMPIE,XMPIELRANK,ISIZE,BC,VC,VG,ISATISFIED,&
            IWHICHSTEN,IPERIODICITY,XPER,YPER,ZPER,ISSF,ISHYAPE,XMPIL,CENTERR)
        IF (LOWMEM.EQ.1)
            CALL STENCIILS_EES(N,IELEM,ILOCALALLELG,TYPESTEN,ILOCALSTENCIL,NUMNEIGHBOURS,ISELEMT,XMPIE,XMPIELRANK,ISIZE,BC,VC,VG,ISATISFIED,&
            IWHICHSTEN,IPERIODICITY,XPER,YPER,ZPER,ISSF,ISHYAPE,XMPIL,CENTERR)
    END IF
    !$OMP END PARALLEL
    if (ees.eq.3)then
...
if (n.eq.0)
    WRITE(100+N,*)CPUX3(1)-CPUX2(1)
CPUX2(1) = MPI_Wtime()
if (n.eq.0)
    print*,"UCNS3D Running"
